function main() {
    ...
    let isThicknessMode = false; // Modo inicial como "cor"

    // Modifique a função handleKeyPress
    function handleKeyPress(event) {
        if (event.key === 'e' || event.key === 'E') {
            isThicknessMode = true; // Ativa o modo de espessura
            console.log("Modo de espessura ativado");
        } else if (event.key === 'k' || event.key === 'K') {
            isThicknessMode = false; // Ativa o modo de cor
            console.log("Modo de cor ativado");
        } else if (event.key >= '0' && event.key <= '9') {
            if (isThicknessMode) {
                // Se o modo de espessura está ativo, muda a espessura
                currentOffset = offsetMap[event.key];
                console.log("Espessura ajustada para:", currentOffset);
            } else {
                // Caso contrário, muda a cor
                currentColor = colorMap[event.key];
                console.log("Cor ajustada para:", currentColor);
            }
    ...
    function drawBresenhamLine(start, end) {
        ...
        points.forEach(([x, y]) => {
            // Aumenta a espessura aplicando o offset aos pontos
            for (let offsetX = -currentOffset; offsetX <= currentOffset; offsetX += 0.01) {
                for (let offsetY = -currentOffset; offsetY <= currentOffset; offsetY += 0.01) {
                    gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
                    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array([x + offsetX, y + offsetY]), gl.STATIC_DRAW);
                    gl.bindBuffer(gl.ARRAY_BUFFER, colorBuffer);
                    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(currentColor), gl.STATIC_DRAW);
                    gl.drawArrays(gl.POINTS, 0, 1);
                }

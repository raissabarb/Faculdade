...
//   return NULL;
    ...
    struct proc **rdy_head;				 //	Processo pronto para executar de cada fila
    int q;                               // Identifica a fila de prioridade
    int total_tickets = 0;               // Total de tickets distribuidos 
    int ticket_count = 0;                // Contador de tickets

    // Calcula o total de tickets distribuidos 
    rdy_head = get_cpulocal_var(run_q_head);
    for (q = 0; q < NR_SCHED_QUEUES; q++) {
        rp = rdy_head[q];
        while (rp) {
            // fila 0 : 16 tickets, fila 1: 15 tickets... fila 15: 1
            total_tickets += (NR_SCHED_QUEUES - q);
            rp = rp->p_nextready;
        }
    }

    // Se nao houver processos prontos, retorna NULL
    if (total_tickets == 0) {
        return NULL;
    }

    // Sorteia um numero aleatorio entre 0 e o total de tickets
    int winning_ticket = random() % total_tickets;

    // Percorre os processos contando os tickets para encontrar o processo com o ticket sorteado
    for (q = 0; q < NR_SCHED_QUEUES; q++) {
        rp = rdy_head[q];
        while (rp) {
            ...
            if (ticket_count > winning_ticket) {
                ...
                return rp; // Processo a ser executado
            }
            rp = rp->p_nextready;
        }
    }

    // Se nenhum processo for selecionado, retorna NULL
    return NULL;
}
